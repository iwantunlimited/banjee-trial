...
export default function Thoughts() {
	const { themeData, setModalData, setModalOpen } =
	...
	const [adminThought, setAdminThought] = useState(true);
	...
	const getThoughtsApiCall = React.useCallback(() => {
		...
		getThoughts({
			...
			.then((res) => {
				...
				res?.content?.map((ele) => {
					if (ele?.user?.userType === -1) {
						setAdminThought(false);
					}
				});
			})
			.finally(() => {
