...
function CustomerView(props) {
	...
	const penddingConnectionsListApiCall = React.useCallback(() => {
		penddingConnectionsList(UserId)
			.then((response) => {
						...
						<Grid item container xs={12} sm={12} md={12} lg={12}>
							...
							>
								...
								>
									...
									>
										<Avatar
											alt={state && state.name && state.name.length>0 ? state.name.slice(0,1): 'A'}
											src={
										...
										<Typography
											...
											variant="h6"
										>
										{window.innerWidth > 1282 ? state && state.email : state && state.email && state.email.slice(0,20) }
										</Typography>
										{
											...
											<Typography
												...
												variant="h6"
											>
											{state && state.email && state.email.slice(20,state.email.length+1)}
											</Typography>
										}
										<Typography
											style={{ marginTop: "5px", color: "grey" }}
										...
										<Grid container spacing={2}>
											{
												...
												conData.content.map((ele, index) => (
													...
													>
														...
														<Badge
															badgeContent={ele && ele.connections.length>0 ? ele.connections.length : 0}
															max="9"
															color="primary"s
                                                            className='badge-css'
                                                        >
															<Avatar
																alt={ele.name && ele.name.length > 0 ? ele.name.slice(0, 1) : 'avatar'}
																src={
																	"https://gateway.banjee.org//services/media-service/iwantcdn/resources/" +
																	ele.avtarUrl
																}
